...
> [!NOTE]
> **AWS コストについて**
> このシステムで使用する AWS サービス（API Gateway・Lambda・SNS・SQS・CloudWatch）はすべて無料枠の範囲内に収まるよう設計されています。重い処理（ダウンロード・変換）はすべてローカルの Docker ワーカで行うため、個人利用であれば **AWS の利用料は実質 $0** です。

### 推奨ホスト環境

Docker ワーカは **Raspberry Pi 4**（4GB RAM、Ubuntu Server 64-bit）での常時稼働に適しています。消費電力が低く（アイドル時 3〜5W 程度）、`arm64` Docker イメージのビルドにも対応しており、自宅ネットワークの日本 IP を維持できるため Radiko・TVer の地域制限にも引っかかりません。
ダウンロードした音声ファイルはスマートフォンで、動画ファイルはDLNAサーバ等で利用することを想定しています。
...
4. `radiko-downloader/` 内で `python get_token.py` を実行します（同じディレクトリに `client_secret.json` が必要）。ブラウザで OAuth 認証を完了すると、`radiko-downloader/` に `token.json` が生成されます。*(注: `client_secret.json` は実行環境には含めないでください)。*
...
Chrome ウェブストアに公開する予定はないため、ローカルで読み込む必要があります。
...
すでにダウンロード済みの TVer / YouTube の URL は通常スキップされます。ダウンロード
ディレクトリに完成済みのファイルが残っていることを `yt-dlp` が検知し、何もしないため
です。`"force": true` を指定すると、それを無視して再度ダウンロードします。最初の
ダウンロードが失敗していた場合や、別の画質で取り直したい場合に便利です。
...
Chrome 拡張機能では同じフラグを **Force re-download** チェックボックスとして提供して
います（ポップアップを閉じるとリセットされます）。

注意点：

- **TVer と YouTube のみが対象です。** Radiko と らじる★らじる の録音はファイルサイズが
  小さく、ラジオワーカーは毎回ダウンロードし直すため、そもそも上書きすべき重複防止の
  仕組みがありません。これらの URL ではこのフラグは無視されます。
- **ダウンロード開始前に既存ファイルが削除されます**（`--force-overwrites`）。その
  ため強制ダウンロードが失敗すると、古いファイルも新しいファイルも残りません。
- **JSON の boolean `true` のみを受け付けます。** それ以外は 400 で拒否されます。この
  フラグは固定の `yt-dlp` 引数を選択するだけで、リクエストの値が `yt-dlp` にそのまま
  渡されることはありません。
- 重複判定はファイル名ベースです。完成したファイルをダウンロードディレクトリの外へ
  移動した場合や、同じ URL を異なる `description` で配信した場合は、ファイル名が変わる
  ためフラグなしでも再ダウンロードされます。
...
`radiko-downloader/.env` と `tver-downloader/.env` は同様の構造を持ち、堅牢な設定オプションを提供します。
...
| `YT_DLP_AUTO_UPDATE` | いいえ | ワーカー起動のたびに `yt-dlp` とそのプラグインをアップグレードします。デフォルトは `true` です。イメージを再ビルドせずに、サイト側の変更に追従して抽出を動作させ続けます。イメージに同梱されたバージョンで固定したい場合は `false` に設定します。 |
| `YT_DLP_UPDATE_TIMEOUT` | いいえ | 上記アップグレードの制限時間（秒）。超過した場合はインストール済みのバージョンで起動します。デフォルトは `300` です。 |
