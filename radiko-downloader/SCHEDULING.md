...
This guide explains how to use the `download-radiko.py` script and a standard Linux `crontab` to schedule your Radiko time-free recordings. 
...
The `download-radiko.py` script is designed to be run **after** a show has finished broadcasting.
...
5 17 * * 0 d=$(TZ=Asia/Tokyo date -d 'now' +%Y%m%d) && ~/bin/download-radiko.py --station FMJ --desc "Sunday Long Show" ${d}130000 ${d}140000 ${d}150000 ${d}160000 >> ~/logs/radiko_cron.log 2>&1
...
```bash
./download-radiko.py --station FMJ --desc "Test Download" 20260314150000
```
